...
export function RegisterModal() {
  ...
  const { register, loginWithGoogle, setRegisterModalOpen } = useAuth();
      ...
      <div className="relative w-full max-w-[720px] bg-white rounded-2xl shadow-2xl overflow-hidden flex flex-col md:flex-row animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex-1 flex flex-col p-6 sm:p-8 bg-white">
          <div className="w-full max-w-[340px] mx-auto flex flex-col justify-center h-full">
            ...
            <p className="text-center text-gray-500 text-[0.8rem] mt-5">
              Already have an account?{" "}
              <Link 
                to="/auth/login" 
                onClick={() => setRegisterModalOpen(false)}
                className="text-[#8A2BE2] hover:text-[#7b22cc] font-bold hover:underline"
              >
                Sign in
              </Link>
            </p>
